...
class CalendarService: CalendarServiceProviding {
    ...
    func events(from start: Date, to end: Date, calendars ids: [String]) async -> [EventModel] {
        ...
        let ekCalendars = filtered.compactMap { model in
            ...
            store.calendars(for: .reminder).first { $0.calendarIdentifier == model.id }
        }

        var events: [EventModel] = []

        if hasAccess(to: .event) {
            let eventCals = ekCalendars.filter { store.calendars(for: .event).contains($0) }
            let predicate = store.predicateForEvents(withStart: start, end: end, calendars: eventCals)
            events.append(contentsOf: store.events(matching: predicate).compactMap { EventModel(from: $0) })
        }

        if hasAccess(to: .reminder) {
            let reminderCals = ekCalendars.filter { store.calendars(for: .reminder).contains($0) }
            events.append(contentsOf: await fetchReminders(from: start, to: end, calendars: reminderCals))
        }
    ...
    private func fetchReminders(from start: Date, to end: Date, calendars: [EKCalendar]) async -> [EventModel] {
        await withCheckedContinuation { continuation in
            ...
            store.fetchReminders(matching: predicate) { reminders in
                ...
                continuation.resume(returning: filtered.compactMap { EventModel(from: $0) })
            }
        }
    }
...
extension EventType {
    init(from event: EKEvent) {
        self = event.birthdayContactIdentifier != nil
            ? .birthday
            : .event(.init(from: event.attendees?.first(where: { $0.isCurrentUser })?.participantStatus))
    }
}
...
extension Array where Element == Participant {
    init(from event: EKEvent) {
        ...
        self.init(participants.map { .init(from: $0, isOrganizer: $0.url == event.organizer?.url) })
    }
}
...
private extension EKCalendar {
    var accountTitle: String {
        ...
        case .local, .subscribed, .birthdays: return "Other"
        default: return source.title
        }
    }

    var isDelegate: Bool {
        if #available(macOS 13.0, *) { return source.isDelegate } else { return false }
    }
}
...
private extension EKEvent {
    var shouldBeAllDay: Bool {
        ...
        let endOfDay = cal.dateInterval(of: .day, for: endDate)?.end
